use secp256k1::hashes::{sha256, hash160, Hash};
...
impl Preimage{
    pub fn new()->Preimage{
        ...
        let sha256_bytes =  sha256::Hash::hash(&hex::decode(preimage.clone()).unwrap()).to_byte_array();
        let hash160_bytes =  hash160::Hash::hash(&hex::decode(preimage.clone()).unwrap()).to_byte_array();
