    ...
    fn test_bitcoin_rsi() {
        ...
        assert!(response.as_ref().unwrap().validate_preimage(preimage.clone().preimage));
        ...
        let mut swap_tx_elements = OnchainSwapTxElements::new(redeem_script_string, RETURN_ADDRESS.to_string(), absolute_fees, Network::Testnet);
